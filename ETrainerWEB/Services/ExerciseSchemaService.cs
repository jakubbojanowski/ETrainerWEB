...
namespace ETrainerWEB.Services
{
    public class ExerciseSchemaService
    {
        ...
        public async Task<List<ExerciseSchemaDTO>> GetExerciseSchemas()
        {
            ...
            var exerciseSchema = await _db.ExerciseSchemas.Where(e => e.User.Id == _userId).ToListAsync();
        ...
        public async Task<int> AddExerciseSchema(ExerciseSchemaDTO exerciseSchemaDTO)
        {
            ...
            var exist = await _db.ExerciseSchemas.FirstOrDefaultAsync(c => c.User.Id == _userId && c.Properties == exerciseSchemaDTO.Properties);
        ...
        { 
            if (string.IsNullOrEmpty(_userId)) return 0;
            exerciseSchemaDTO.User = await _db.Users.FirstOrDefaultAsync(e => e.Id == _userId);
            var exerciseSchema = await _db.ExerciseSchemas.FirstOrDefaultAsync(e =>e.User.Id == _userId &&e.Id == exerciseSchemaDTO.Id);
        ...
        public async Task<bool> DeleteExerciseSchema(int id)
        {
            ...
            var exerciseSchema = await _db.ExerciseSchemas.FirstOrDefaultAsync(e => e.User.Id == _userId && e.Id == id);
